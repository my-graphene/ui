...
// Filter RUI 0.9.2+ import keys export file so that it will include only private keys
// that may be found in the RUI 2.0 genesis block.
// Dependencies:
...
* The bloom filter was created with a genesis containing RUI prefixed keys.  Create
...
fs.readFile('bloom_bitshares.dat', function (err, data) {
    ...
    function in_bloom(str) {
        // echo -n '0:RUI87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3m'|sha256sum
        // str = 'RUI87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3n' // should be found
        // str = 'RUI4A43UCoWz1F5vqLxX3LLoGHKs1GzCGRjZ' // should be found
    ...
    stdin.on('end', function () {
        ...
        for(let a = 0; a < wallet.account_keys.length; a++) {
            ...
            for(var k = keys.encrypted_private_keys.length - 1; k >= 0; k--) {
                ...
                if( /^RUI/.test(key) ) key = chainPrefix + key.substring(3)
