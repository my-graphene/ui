...
then
    ## wallet.ruichain.org subdomain (independent repo)
    echo "Pushing new wallet subdomain repo"
    git clone https://github.com:${GITHUB_TOKEN}@github.com/${WALLET_REPO} $TRAVIS_BUILD_DIR/wallet.ruichain.org
    cd $TRAVIS_BUILD_DIR/wallet.ruichain.org
...
then
    ## staging.ruichain.org subdomain (independent repo)
    echo "Pushing new staging subdomain repo"
    git clone https://github.com:${GITHUB_TOKEN}@github.com/${STAGING_REPO} $TRAVIS_BUILD_DIR/staging.ruichain.org
    cd $TRAVIS_BUILD_DIR/staging.ruichain.org
    rm -rf ./*
    git checkout ./CNAME
    cp -Rv $TRAVIS_BUILD_DIR/build/hash-history/* .
    git add -A
...
then
    ## develop.ruichain.org subdomain (independent repo)
    echo "Pushing new develop subdomain repo"
    git clone https://github.com:${GITHUB_TOKEN}@github.com/${DEVELOP_REPO} $TRAVIS_BUILD_DIR/develop.ruichain.org
    cd $TRAVIS_BUILD_DIR/develop.ruichain.org
    rm -rf ./*
    git checkout ./CNAME
    cp -Rv $TRAVIS_BUILD_DIR/build/hash-history/* .
    git add -A
