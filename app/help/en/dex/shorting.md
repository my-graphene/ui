...
In order to increase your exposure to RUI and offer liquidity to BitAssets, such
...
The RuiChain network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 RUI as it is traded on external exchanges.
...
The RuiChain network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/RUI).
...
market rates up to the SQP and close the position. The remaining RUI of the
...
increase, an additional amount of RUI is locked as collateral, while reducing
...
particular bitAsset to hand it over to the RuiChain network. After that, the
