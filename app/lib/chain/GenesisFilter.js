...
var token_genesiskeys_bloom_url = undefined;
try {
    var url = require("file-loader?name=token_genesiskeys_bloom_[sha1:hash:hex:7].dat!assets/token_genesiskeys_bloom.dat");
    if (url.indexOf("3cee441") === -1)
        throw new Error("Incorrect hash: token_genesiskeys_bloom.dat");
    token_genesiskeys_bloom_url = url;
} catch (e) {
    // webpack deployment exception (not run time)
    console.log(
        "WARN: Will be unable to filter RUI 1.0 wallet imports, did not find assets/token_genesiskeys_bloom.dat",
        e
    );
}
...
export default class GenesisFilter {
    ...
    isAvailable() {
        return token_genesiskeys_bloom_url !== undefined;
    }
    ...
    init(done) {
        ...
        xhr.open("GET", token_genesiskeys_bloom_url);
        xhr.send();
    }
